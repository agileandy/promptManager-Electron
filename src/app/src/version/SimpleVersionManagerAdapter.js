/**
 * Simple Version Manager Adapter
 *
 * This adapter provides the SimpleVersionManager functions directly as ES module exports
 * to be compatible with dynamic imports in the browser environment.
 *
 * This avoids the need to change the underlying architecture while fixing the
 * module compatibility issue.
 */

// Delete prompt with version resequencing
export async function deletePromptWithResequencing(db, renderPrompts, promptId) {
    try {
...
export async function resequenceAllVersions(db) {
    try {
...
export async function deletePromptVersion(db, versionId) {
    try {
